...
当前开发机需要先补齐 Rust、Cargo 和 Windows MSVC Build Tools，才能运行桌面模式。

## 数据备份

应用内提供“导出备份”和“导入备份”入口。

- 浏览器模式会导出 localStorage 中的 OpenBill 数据
- 桌面模式会导出 SQLite 中的 OpenBill 数据
- 导入备份会覆盖当前本地数据，导入前应用会弹出确认

备份文件是 JSON 格式，适合手动保存、迁移设备或未来导入服务器模式。
